<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Blog List') }}
            <span><a href="{{ url('blog-list/create') }}"><button class="float-right text-blue-800">Add</button></a></span>
        </h2>
    </x-slot>
                    ...
                    @foreach($blogs as $blog)
                        <tr class="bg-white lg:hover:bg-gray-100 flex lg:table-row flex-row lg:flex-row flex-wrap lg:flex-no-wrap mb-10 lg:mb-0">
                            ...
                            <td class="w-full lg:w-auto p-3 text-gray-800 border border-b text-center block lg:table-cell relative lg:static">
                                ...
                                <a href="{{ url('blog-list/'.$blog->id.'/edit') }}" class="text-blue-400 hover:text-blue-600 underline pl-6">Edit</a>
                                <form action="{{ url('blog-list/'.$blog->id) }}" method="post">
                                    @method('delete')
                                    @csrf
